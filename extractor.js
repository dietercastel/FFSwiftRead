function initExtract(_paramInstance) {
    ...
    $D.parseOptions =
    {
        '_elements_ignore': '|button|input|select|textarea|optgroup|command|datalist|--|frame|frameset|noframes|--|style|link|script|noscript|--|canvas|applet|map|--|marquee|area|base|--|img|figure|cite|header|footer|aside|time|label|',
        ...
        '_elements_ignore_with_attribute':
        {
            'div': {
                'class': '|post-gallery|module|headline|yog-col|yom-figure|caption|comments-view|cnnExplainer|widget-container|email-subscription-promo|asset-metabar|story-asset|story-feature|comment-introduction|video-bar|shareArticles|article-reader-comments|mod-inline|chart|',
                'id': '|connect|'
            }, //for wsj, yahoo, cnn, nyt
            'nav': { 'id': '|next-in|' }
        },
    ...
    $D.detectLanguage = function () {
        ...
        var _add_text = function (_elements) {
            ...
            //  add
            for (var _l = _elements.length, _i = 0, _ii = Math.min(5, _l); _i < _ii; _i++) { _text += ' ' + _elements[$D.rand(1, _l) - 1].innerHTML.replace(/<([^>]+?)>/gi, '').replace(/([ \n\r\t]+)/gi, ' ').substr(0, 150); }
        };
        ...
        var _paragraphs = $D.document.getElementsByTagName('p'),
            ...
            _body = $D.document.getElementsByTagName('body');
            ...
            switch (true) {
                ...
                case (_tag_name == 'a'):
                    ...
                    for (var i = 0, _i = $D.parseOptions._skip_link_from_domain.length; i < _i; i++) {
                        if (_node.href.indexOf($D.parseOptions._skip_link_from_domain[i]) > -1) { _result._is__link_skip = true; break; }
                    }
                ...
                case (_tag_name == 'img'):
                    ...
                    if (_node.src && _node.src.indexOf) {
                        for (var i = 0, _i = $D.parseOptions._skip_image_from_domain.length; i < _i; i++) {
                            if (_node.src.indexOf($D.parseOptions._skip_image_from_domain[i]) > -1) { _result._is__image_skip = true; break; }
                        }
                    }
    ...
    $D.getContent__buildHTMLForNode = function (_nodeToBuildHTMLFor, _custom_mode) {
        ...
        var _recursive = function (_node) {
            ...
            //  ignore tags with attributes
            //  ===========================

            if (_tag_name in $D.parseOptions._elements_ignore_with_attribute) {
                //  for each attribute specified
                for (var _attribute_type in $D.parseOptions._elements_ignore_with_attribute[_tag_name]) {
                    _attribute_values_to_ignore = $D.parseOptions._elements_ignore_with_attribute[_tag_name][_attribute_type];
                    _attribute_value = _node.getAttribute(_attribute_type);

                    if (_attribute_value == null)
                        continue;

                    //  it could be a class attr with several tags
                    _attribute_values = _attribute_value.split(' ');
                    for (var i = 0; i < _attribute_values.length; i++) {
                        _current_attriute_value = _attribute_values[i]
                        //  if attribute value, e.g. class name, is in the list to ignore
                        if (_attribute_values_to_ignore.indexOf('|' + _current_attriute_value + '|') > -1) {
                            //console.log('ignoring '+_tag_name+' '+_attribute_type+' '+_attribute_value);
                            return;
                        }
                    }
                }
            ...
            switch (_tag_name) {
                ...
                case ('iframe'):
                    ...
                    if (_skip) { } else {
                        ...
                        //  loop
                        for (var i = 0, _i = $D.parseOptions._keep_video_from_domain.length; i < _i; i++) { if (_src.indexOf($D.parseOptions._keep_video_from_domain[i]) > -1) { _skip = false; break; } }
                    }
            ...
            if ($D.parseOptions._elements_ignore_tag.indexOf('|' + _tag_name + '|') > -1) { } else {
                ...
                if (_id_attribute > '') { _global__the_html += ' id="' + _id_attribute + '"'; }
            ...
            if ($D.parseOptions._elements_self_closing.indexOf('|' + _tag_name + '|') > -1) { } else {
                for (var i = 0, _i = _node.childNodes.length; i < _i; i++) { _recursive(_node.childNodes[i]); }
            }
    ...
    $D.getContent__find__getIsolatedTitleInHTML = function (_html) {
        ...
        //  regex
        var _getTitleRegex = new RegExp($D.articleTitleMarker__start + '(.*?)' + $D.articleTitleMarker__end, 'i'),
...
function store_extracted_result(html) {
    console.log('storing extracted html:', html);
    chrome.runtime.sendMessage({ action: "extractor", html: html }, function (response) {
        //console.log(response.farewell);
    });
}

function _processExtractedHtml(html) {
    // console.log('extracted html to process: ', html);
    //add spaces after all </div>s. because paragraphs are often separated by divs, but there's no extra space
...
function run() {
    ...
    let extractObj = {
        'callbacks': {
            'finished': function (_result) {
                /* set result */ extract_result = _result;
                // console.log(extract_result._html);
                html = _processExtractedHtml(extract_result._html);
                                 // console.log('processed html to store: ', html);
                /* callback */   store_extracted_result(html);
            }
        },
        'debug': false,
        'window': window,
    ...
    console.log('extraction object: ', extractObj);

    let extract = initExtract(extractObj);
    console.log('starting text extraction...');
    extract.start();

}
if (typeof (waitForJquery) == 'undefined') {
    waitForJquery = undefined;
}
